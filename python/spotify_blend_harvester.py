...
def retrieve_blend_information() -> list[dict[str, str]]:
    ...
    while scroll:
        ...
        blends = [
            ...
            and str(element.get("content-desc")).split(" + ")[0]
            not in [list(item.keys())[0] for item in done_blends]  # prevent duplicates
        ]
        ...
        logging.debug(
            f"Scroll region: ({last[0]}, {last[1]}) -> ({first[0]}, {first[1]})"
        )
